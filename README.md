# AI-Powered Prepared with Hygiene Certification
...
* **[span_15](start_span)Periodic Snapshots:** The system extracts frames from pre-recorded test clips at a set rate, such as 1 to 2 Frames Per Second[span_15](end_span).
* **[span_16](start_span)Efficiency:** This discards redundant information, like a static camera capturing an empty prep station, and focuses the AI's token limit strictly on moments of action[span_16](end_span).
...
* **[span_21](start_span)Actionable Alerts:** The notification is not a vague warning[span_21](end_span). It includes specific Evidence Markers:
  * [span_22](start_span)The exact timestamp and visual frame of the recorded violation[span_22](end_span).
  * [span_23](start_span)A summary of the AI debate, clearly explaining the reason why the policy was breached[span_23](end_span).

## Technical Stack and Architecture
* **[span_24](start_span)Frontend:** Uses React to display the "Prepared with Hygiene" badge and verifiable status to the end consumer[span_24](end_span).
* **[span_25](start_span)Data Ingestion:** Uses Python, OpenCV, and FFmpeg to process pre-recorded video clips and extract periodic frames at 1-2 FPS for the AI[span_25](end_span).
* **[span_26](start_span)AI Backend:** Utilizes Multi-Agent LLMs to facilitate the Inspector, Auditor, and Judge debate and output a binary classification[span_26](end_span).
* **[span_27](start_span)Intervention:** Leverages the Telegram Bot API to automatically push timestamped evidence and debate summaries to restaurant managers[span_27](end_span).
* **[span_28](start_span)Collaboration:** Uses Git and Google Sheets as the current active workspace for the team to manage prompts, policies, and code versioning[span_28](end_span).
